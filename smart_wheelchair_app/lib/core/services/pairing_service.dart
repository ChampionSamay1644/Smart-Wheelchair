...
class PairingService {
  ...
  static Future<String> generatePairCode({
  ...
  static Future<void> claimPairCode(String code) async {
    ...
    await _firestore.runTransaction((tx) async {
      ...
      if (expires != null && expires.toDate().isBefore(DateTime.now()))
        throw PairingError('Code expired');
      if (ownerUid == null) throw PairingError('Invalid code owner');
      if (ownerUid == claimer.uid)
        throw PairingError('Cannot claim your own code');
      ...
      if (!ownerSnap.exists || !claimerSnap.exists)
        throw PairingError('User documents not found');
      ...
      if (ownerRole == 'guardian' && claimerType == 'patient') {
        ...
        tx.update(claimerRef, {'guardianId': ownerUid});
        ...
        tx.update(ownerRef, {'guardianId': claimer.uid});
      } else {
        throw PairingError(
          'Role mismatch: cannot pair $ownerRole with $claimerType',
        );
      }
